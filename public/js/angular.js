 ...
 */
  let lowercase = function (string) { return isString(string) ? string.toLowerCase() : string; };
 ...
 */
  let uppercase = function (string) { return isString(string) ? string.toUpperCase() : string; };


  const manualLowercase = function (s) {
  /* jshint bitwise: false */
    return isString(s)
      ? s.replace(/[A-Z]/g, ch => String.fromCharCode(ch.charCodeAt(0) | 32))
      : s;
  };
  const manualUppercase = function (s) {
  /* jshint bitwise: false */
    return isString(s)
      ? s.replace(/[a-z]/g, ch => String.fromCharCode(ch.charCodeAt(0) & ~32))
      : s;
 ...
 */
  ...
  function inherit(parent, extra) {
    return extend(new (extend(() => {}, { prototype: parent }))(), extra);
  }
 ...
 */
  function copy(source, destination, stackSource, stackDest) {
    ...
    if (!destination) {
      ...
      if (isObject(source)) {
        const index = indexOf(stackSource, source);
 ...
 */
  ...
  var csp = function () {
    ...
    let active = !!(document.querySelector('[ng-csp]') ||
 ...
 */
  function startingTag(element) {
    ...
    const TEXT_NODE = 3;
    const elemHtml = jqLite('<div>').append(element).html();
    try {
      return element[0].nodeType === TEXT_NODE ? lowercase(elemHtml) :
        elemHtml
          .match(/^(<[^>]+>)/)[1]
          .replace(/^<([\w\-]+)/, (match, nodeName) => `<${lowercase(nodeName)}`);
    } catch (e) {
 ...
 */
  function parseKeyValue(/** string */keyValue) {
    let obj = {}, key_value, key;
    forEach((keyValue || '').split('&'), (keyValue) => {
      if (keyValue) {
        key_value = keyValue.replace(/\+/g, '%20').split('=');
        key = tryDecodeURIComponent(key_value[0]);
        if (isDefined(key)) {
          const val = isDefined(key_value[1]) ? tryDecodeURIComponent(key_value[1]) : true;
    ...
    forEach(elements, (element) => {
      if (!appElement) {
        ...
        if (match) {
          ...
          forEach(element.attributes, (attr) => {
 ...
 */
  function bootstrap(element, modules) {
    const doBootstrap = function () {
      ...
      if (element.injector()) {
        const tag = (element[0] === document) ? 'document' : startingTag(element);
     ...
     */
      return function module(name, requires, configFn) {
        ...
        return ensure(modules, name, () => {
          ...
          const config = invokeLater('$injector', 'invoke');
 ...
 */
  const jqData = JQLite._data = function (node) {
 ...
 */
  ...
  function jqLitePatchJQueryRemove(name, dispatchThis, filterElems, getterIfNoArguments) {
    let originalJqFn = jQuery.fn[name];
    ...
    // jshint -W040
      let list = filterElems && param ? [this.filter(param)] : [this],
  ...
  function jqLiteOff(element, type, fn, unsupported) {
    ...
    let events = jqLiteExpandoStore(element, 'events'),
  ...
  function jqLiteData(element, key, value) {
    let data = jqLiteExpandoStore(element, 'data'),
  ...
  // ////////////////////////////////////////
  const BOOLEAN_ATTR = {};
  forEach('multiple,selected,checked,disabled,readOnly,required,open'.split(','), (value) => {
    BOOLEAN_ATTR[lowercase(value)] = value;
  });
  const BOOLEAN_ELEMENTS = {};
  forEach('input,select,option,textarea,button,form,details'.split(','), (value) => {
   ...
   */
    JQLite.prototype[name] = function (arg1, arg2) {
      ...
      if (fn !== jqLiteEmpty &&
        ...
        const jj = (value === undefined) ? Math.min(nodeCount, 1) : nodeCount;
        for (let j = 0; j < jj; j++) {
  ...
  forEach({
        ...
        if (!eventFns) {
          if (type == 'mouseenter' || type == 'mouseleave') {
            ...
            onFn(element, eventmap[type], function (event) {
              let target = this, related = event.relatedTarget;
    ...
    replaceWith(element, replaceNode) {
      let index, parent = element.parentNode;
    ...
    after(element, newElement) {
      let index = element, parent = element.parentNode;
      forEach(new JQLite(newElement), (node) => {
        parent.insertBefore(node, index.nextSibling);
        index = node;
      });
    },
    ...
    triggerHandler(element, event, extraParameters) {
      ...
      const eventFns = (jqLiteExpandoStore(element, 'events') || {})[eventName];
 ...
 */
   ...
   */
    remove(key) {
      const value = this[key = hashKey(key, this.nextUid)];
      delete this[key];
      return value;
    }
  };
  ...
  function createInjector(modulesToLoad) {
    var INSTANTIATING = {},
      ...
      instanceInjector = (instanceCache.$injector =
          createInternalInjector(instanceCache, (servicename) => {
            const provider = providerInjector.get(servicename + providerSuffix);
            return instanceInjector.invoke(provider.$get, provider);
          }));


    forEach(loadModules(modulesToLoad), (fn) => { instanceInjector.invoke(fn || noop); });
    ...
    function loadModules(modulesToLoad) {
      let runBlocks = [], moduleFn, invokeQueue, i, ii;
      ...
      function getService(serviceName) {
        if (cache.hasOwnProperty(serviceName)) {
          if (cache[serviceName] === INSTANTIATING) {
            throw $injectorMinErr(
              'cdep', 'Circular dependency found: {0}',
 ...
 */
  function Browser(window, document, $log, $sniffer) {
    ...
    const outstandingRequestCallbacks = [];
   ...
   */
    ...
    // Android Browser BFCache causes location, history reference to become stale.
      ...
      if (url) {
        ...
        const sameBase = lastBrowserUrl && stripHash(lastBrowserUrl) === stripHash(url);
        lastBrowserUrl = url;
   ...
   */
    ...
    const cookiePath = self.baseHref();
   ...
   */
    ...
    /* global escape: false, unescape: false */
      let cookieLength, cookieArray, cookie, i, index;
   ...
   */
    ...
    this.$get = [
      ...
      function (
        ...
        const Attributes = function (element, attr) {
       ...
       */
          $set(key, value, writeAttr, attrName) {
            ...
            let booleanKey = getBooleanAttrName(this.$$element[0], key),
              normalizedVal,
              nodeName;
            ...
            $$observers && forEach($$observers[key], (fn) => {
              try {
                fn(value);
              } catch (e) {
                $exceptionHandler(e);
       ...
       */
        ...
        function compile(
          ...
          forEach($compileNodes, (node, index) => {
            if (node.nodeType == 3 /* text node */ && node.nodeValue.match(/\S+/) /* non-empty */) {
              $compileNodes[index] = node = jqLite(node).wrap('<span></span>').parent()[0];
            }
          });
          const compositeLinkFn =
              compileNodes(
                $compileNodes, transcludeFn, $compileNodes,
          ...
          return function publicLinkFn(scope, cloneConnectFn, transcludeControllers, parentBoundTranscludeFn) {
            ...
            const $linkNode = cloneConnectFn
              ? JQLitePrototype.clone.call($compileNodes) // IMPORTANT!!!
              : $compileNodes;

            forEach(transcludeControllers, (instance, name) => {
              $linkNode.data(`$${name}Controller`, instance);
            });

            // Attach scope only to non-text nodes.
            for (let i = 0, ii = $linkNode.length; i < ii; i++) {
              let node = $linkNode[i],
     ...
     */
        function compileNodes(
          ...
          for (let i = 0; i < nodeList.length; i++) {
            ...
            directives = collectDirectives(
              nodeList[i], [], attrs, i === 0 ? maxPriority : undefined,
              ignoreDirective
            );

            nodeLinkFn = (directives.length)
              ? applyDirectivesToNode(
                directives, nodeList[i], attrs, transcludeFn, $rootElement,
          ...
          function compositeLinkFn(scope, nodeList, $rootElement, parentBoundTranscludeFn) {
            let nodeLinkFn, childLinkFn, node, childScope, i, ii, n, childBoundTranscludeFn;
        ...
        function createBoundTranscludeFn(scope, transcludeFn, previousBoundTranscludeFn) {
          const boundTranscludeFn = function (transcludedScope, cloneFn, controllers) {
            ...
            const clone = transcludeFn(transcludedScope, cloneFn, controllers, previousBoundTranscludeFn);
            if (scopeCreated) {
              clone.on('$destroy', () => { transcludedScope.$destroy(); });
            }
            return clone;
          };
            ...
            case 1: /* Element */
              ...
              for (var attr, name, nName, ngAttrName, value, isNgAttr, nAttrs = node.attributes,
                ...
                if (!msie || msie >= 8 || attr.specified) {
                  ...
                  const directiveNName = ngAttrName.replace(/(Start|End)$/, '');
     ...
     */
        function applyDirectivesToNode(
          ...
          for (let i = 0, ii = directives.length; i < ii; i++) {
            ...
            if (directiveValue = directive.scope) {
              ...
              if (!directive.templateUrl) {
                assertNoDuplicate(
                  'new/isolated scope', newIsolateScopeDirective, directive,
                  $compileNode
                );
            ...
            if (directiveValue = directive.transclude) {
              ...
              if (directiveValue == 'element') {
                ...
                childTranscludeFn = compile(
                  $template, transcludeFn, terminalPriority,
            ...
            if (directive.template) {
              ...
              if (directive.replace) {
                ...
                const templateDirectives = collectDirectives(compileNode, [], newTemplateAttrs);
            ...
            if (directive.templateUrl) {
              ...
              nodeLinkFn = compileTemplateUrl(
                directives.splice(i, directives.length - i), $compileNode,
          ...
          function getControllers(directiveName, require, $element, elementControllers) {
            let value, retrievalMethod = 'data', optional = false;
            if (isString(require)) {
          ...
          function nodeLinkFn(childLinkFn, scope, linkNode, $rootElement, boundTranscludeFn) {
            let attrs, $element, i, ii, linkFn, controller, isolateScope, elementControllers = {}, transcludeFn;
            ...
            if (newIsolateScopeDirective) {
              ...
              forEach(newIsolateScopeDirective.scope, (definition, scopeName) => {
            ...
            if (controllerDirectives) {
              forEach(controllerDirectives, (directive) => {
                let locals = {
            ...
            for (i = 0, ii = preLinkFns.length; i < ii; i++) {
              try {
                linkFn = preLinkFns[i];
                linkFn(
                  linkFn.isolateScope ? isolateScope : scope, $element, attrs,
     ...
     */
        ...
        function compileTemplateUrl(
          ...
          $http.get($sce.getTrustedResourceUrl(templateUrl), { cache: $templateCache })
            .success((content) => {
              let compileNode, tempTemplateAttrs, $template, childBoundTranscludeFn;
              ...
              if (origAsyncDirective.replace) {
                ...
                const templateDirectives = collectDirectives(compileNode, [], tempTemplateAttrs);
            ...
            .error((response, code, headers, config) => {
     ...
     */
        ...
        function assertNoDuplicate(what, previousDirective, directive, element) {
          if (previousDirective) {
            throw $compileMinErr(
              'multidir', 'Multiple directives [{0}, {1}] asking for {2} on: {3}',
          ...
          if (interpolateFn) {
            directives.push({
              ...
              // then we don't have a parent and should do this in the linkFn
                let parent = templateNode.parent(), hasCompileParent = parent.length;
            ...
            compile() {
              return {
                pre: function attrInterpolatePreLinkFn(scope, element, attr) {
                  const $$observers = (attr.$$observers || (attr.$$observers = {}));
     ...
     */
        function replaceWith($rootElement, elementsToRemove, newNode) {
          ...
          if ($rootElement) {
            for (i = 0, ii = $rootElement.length; i < ii; i++) {
              if ($rootElement[i] == firstElementToRemove) {
                ...
                for (let j = i, j2 = j + removeCount - 1,
          ...
          for (let k = 1, kk = elementsToRemove.length; k < kk; k++) {
            const element = elementsToRemove[k];
        ...
        function cloneAndAnnotateFn(fn, annotation) {
          return extend(function () { return fn(...arguments); }, fn, annotation);
        }
      }];
  }
 ...
 */
  function headersGetter(headers) {
    let headersObj = isObject(headers) ? headers : undefined;
 ...
 * */
   ...
   */
    const responseInterceptorFactories = this.responseInterceptors = [];
     ...
     */
        ...
        forEach(responseInterceptorFactories, (interceptorFactory, index) => {
          const responseFn = isString(interceptorFactory)
  ...
  it('should make JSONP request to invalid URL and invoke the error handler',
     ...
     */
        function $http(requestConfig) {
          ...
          const chain = [serverRequest, undefined];
          ...
          while (chain.length) {
            const thenFn = chain.shift();
            const rejectFn = chain.shift();
     ...
     */
        function sendReq(config, reqData, reqHeaders) {
          ...
          if (isUndefined(cachedResp)) {
            const xsrfValue = urlIsSameOrigin(config.url)
            ...
            $httpBackend(
              config.method, url, reqData, done, reqHeaders, config.timeout,
       ...
       */
          ...
          function removePendingReq() {
            const idx = indexOf($http.pendingRequests, config);
 ...
 */
  ...
  function createHttpBackend($browser, createXhr, $browserDefer, callbacks, rawDocument) {
    ...
    // - adds and immediately removes script elements from the document
      let script = rawDocument.createElement('script'), callback = null;
        ...
        if (!mustHaveExpression || hasInterpolation) {
          ...
          fn = function (context) {
            try {
              ...
              const newErr = $interpolateMinErr(
                'interr', "Can't interpolate: {0}\n{1}", text,
                err.toString()
              );
 ...
 */
  ...
  function parseAbsoluteUrl(absoluteUrl, locationObj, appBase) {
    const parsedUrl = urlResolve(absoluteUrl, appBase);
  ...
  function parseAppUrl(relativeUrl, locationObj, appBase) {
    const prefixed = (relativeUrl.charAt(0) !== '/');
    if (prefixed) {
      relativeUrl = `/${relativeUrl}`;
    }
    const match = urlResolve(relativeUrl, appBase);
 ...
 */
   ...
   */
    this.$$parse = function (url) {
      const pathUrl = beginsWith(appBaseNoFile, url);
      if (!isString(pathUrl)) {
        throw $locationMinErr(
          'ipthprfx', 'Invalid url "{0}", missing path prefix "{1}".', url,
          appBaseNoFile
        );
      }
   ...
   */
    this.$$compose = function () {
      let search = toKeyValue(this.$$search),
 ...
 */
   ...
   */
    this.$$parse = function (url) {
      const withoutBaseUrl = beginsWith(appBase, url) || beginsWith(appBaseNoFile, url);
      const withoutHashUrl = withoutBaseUrl.charAt(0) == '#'
      ...
      if (!isString(withoutHashUrl)) {
        throw $locationMinErr(
          'ihshprfx', 'Invalid url "{0}", missing hash prefix "{1}".', url,
          hashPrefix
        );
      }
 ...
 */
   ...
   * @return {string} hash
   */
    hash: locationGetterSetter('$$hash', hash => (hash !== null ? hash.toString() : '')),
  ...
  Lexer.prototype = {
    ...
    lex(text) {
      ...
      while (this.index < this.text.length) {
        ...
        if (this.is('"\'')) {
          ...
          const fn = OPERATORS[this.ch];
          const fn2 = OPERATORS[ch2];
          const fn3 = OPERATORS[ch3];
          if (fn3) {
    ...
    throwError(error, start, end) {
      ...
      const colStr = (isDefined(start)
        ...
        : ` ${end}`);
      throw $parseMinErr(
        'lexerr', 'Lexer Error: {0} at column{1} in expression [{2}].',
    ...
    readIdent() {
      ...
      if (OPERATORS.hasOwnProperty(ident)) {
        ...
        const getter = getterFn(ident, this.options, this.text);
        token.fn = extend((self, locals) => (getter(self, locals)), {
          assign(self, value) {
 ...
 */
  ...
  Parser.prototype = {
    ...
    unaryFn(fn, right) {
      return extend((self, locals) => fn(self, locals, right), {
        constant: right.constant
      });
    },

    ternaryFn(left, middle, right) {
      return extend((self, locals) => (left(self, locals) ? middle(self, locals) : right(self, locals)), {
        constant: left.constant && middle.constant && right.constant
      });
    },

    binaryFn(left, fn, right) {
      return extend((self, locals) => fn(self, locals, left, right), {
    ...
    filter() {
      ...
      const fn = this.$filter(token.text);
      const argsFn = [];
      while (true) {
        if ((token = this.expect(':'))) {
    ...
    fieldAccess(object) {
      const parser = this;
      const field = this.expect().text;
      const getter = getterFn(field, this.options, this.text);

      return extend((scope, locals, self) => getter(self || object(scope, locals)), {
        assign(scope, value, locals) {
          let o = object(scope, locals);
    ...
    objectIndex(obj) {
      ...
      return extend((self, locals) => {
        ...
        assign(self, value, locals) {
          const key = ensureSafeMemberName(indexFn(self, locals), parser.text);
          // prevent overwriting of Function.constructor which would break ensureSafeObject check
          let o = ensureSafeObject(obj(self, locals), parser.text);
    ...
    functionCall(fn, contextGetter) {
      ...
      return function (scope, locals) {
        const args = [];
        const context = contextGetter ? contextGetter(scope, locals) : scope;

        for (let i = 0; i < argsFn.length; i++) {
          args.push(ensureSafeObject(argsFn[i](scope, locals), parser.text));
        }
        const fnPtr = fn(scope, locals, context) || noop;
    ...
    arrayDeclaration() {
      ...
      if (this.peekToken().text !== ']') {
        do {
          ...
          const elementFn = this.expression();
  ...
  function setter(obj, path, setValue, fullExp, options) {
    ...
    for (let i = 0; element.length > 1; i++) {
      ...
      let propertyObj = ensureSafeObject(obj[key], fullExp);
      if (!propertyObj) {
        propertyObj = {};
        obj[key] = propertyObj;
      }
      ...
      if (obj.then && options.unwrapPromises) {
        ...
        if (!('$$v' in obj)) {
          (function (promise) {
            promise.then((val) => { promise.$$v = val; });
          }(obj));
        }
        if (obj.$$v === undefined) {
          obj.$$v = {};
  ...
  const getterFnCacheDefault = {};
  const getterFnCacheExpensive = {};
 ...
 */
  function cspSafeGetterFn(key0, key1, key2, key3, key4, fullExp, options) {
    ...
    const eso = function (o) {
      return ensureSafeObject(o, fullExp);
    };
    const expensiveChecks = options.expensiveChecks;
    const eso0 = (expensiveChecks || isPossiblyDangerousMemberName(key0)) ? eso : identity;
    const eso1 = (expensiveChecks || isPossiblyDangerousMemberName(key1)) ? eso : identity;
    const eso2 = (expensiveChecks || isPossiblyDangerousMemberName(key2)) ? eso : identity;
    const eso3 = (expensiveChecks || isPossiblyDangerousMemberName(key3)) ? eso : identity;
    const eso4 = (expensiveChecks || isPossiblyDangerousMemberName(key4)) ? eso : identity;
      ...
      : function cspSafePromiseEnabledGetter(scope, locals) {
        ...
        if (pathVal && pathVal.then) {
          ...
          if (!('$$v' in pathVal)) {
            promise = pathVal;
            promise.$$v = undefined;
            promise.then((val) => { promise.$$v = eso0(val); });
          }
        ...
        if (pathVal && pathVal.then) {
          ...
          if (!('$$v' in pathVal)) {
            promise = pathVal;
            promise.$$v = undefined;
            promise.then((val) => { promise.$$v = eso1(val); });
          }
        ...
        if (pathVal && pathVal.then) {
          ...
          if (!('$$v' in pathVal)) {
            promise = pathVal;
            promise.$$v = undefined;
            promise.then((val) => { promise.$$v = eso2(val); });
          }
        ...
        if (pathVal && pathVal.then) {
          ...
          if (!('$$v' in pathVal)) {
            promise = pathVal;
            promise.$$v = undefined;
            promise.then((val) => { promise.$$v = eso3(val); });
          }
        ...
        if (pathVal && pathVal.then) {
          ...
          if (!('$$v' in pathVal)) {
            promise = pathVal;
            promise.$$v = undefined;
            promise.then((val) => { promise.$$v = eso4(val); });
          }
  ...
  function getterFn(path, options, fullExp) {
    const expensiveChecks = options.expensiveChecks;
    const getterFnCache = (expensiveChecks ? getterFnCacheExpensive : getterFnCacheDefault);
    ...
    if (options.csp) {
      if (pathKeysLength < 6) {
        fn = cspSafeGetterFn(
          pathKeys[0], pathKeys[1], pathKeys[2], pathKeys[3], pathKeys[4], fullExp,
          options
        );
      } else {
        fn = function (scope, locals) {
          ...
          do {
            val = cspSafeGetterFn(
              pathKeys[i++], pathKeys[i++], pathKeys[i++], pathKeys[i++],
              pathKeys[i++], fullExp, options
            )(scope, locals);
      ...
      let needsEnsureSafeObject = expensiveChecks;
      forEach(pathKeys, (key, index) => {
        ...
        const wrapWithEso = expensiveChecks || isPossiblyDangerousMemberName(key);
        if (wrapWithEso) {
      ...
      let evaledFnGetter = new Function('s', 'l', 'pw', 'eso', 'fe', code);
      /* jshint +W054 */
   ...
   */
    var defer = function () {
      ...
      deferred = {
        ...
        promise: {
          then(callback, errback, progressback) {
            const result = defer();
            ...
            const wrappedErrback = function (reason) {
              try {
            ...
            const wrappedProgressback = function (progress) {
              try {
            ...
            function handleCallback(value, isResolved) {
              ...
              if (isPromiseLike(callbackOutput)) {
                return callbackOutput.then(() => makePromise(value, isResolved), error => makePromise(error, false));
              }
              return makePromise(value, isResolved);
            }

            return this.then(value => handleCallback(value, true), error => handleCallback(error, false));
          }
        }
      };
      ...
      */
       ...
       */
          $watch(watchExp, listener, objectEquality) {
            ...
            if (!isFunction(listener)) {
              const listenFn = compileToFn(listener || noop, 'listener');
              watcher.fn = function (newVal, oldVal, scope) { listenFn(scope); };
            }

            if (typeof watchExp === 'string' && get.constant) {
              const originalFn = watcher.fn;
       ...
       */
          $on(name, listener) {
            ...
            const self = this;
            return function () {
              const indexOfListener = indexOf(namedListeners, listener);
 ...
 */
  function $$SanitizeUriProvider() {
    let aHrefSanitizationWhitelist = /^\s*(https?|ftp|mailto|tel|file):/,
 ...
 */
  ...
  const originUrl = urlResolve(window.location.href, true);
 ...
 */
  function urlIsSameOrigin(requestUrl) {
    const parsed = (isString(requestUrl)) ? urlResolve(requestUrl) : requestUrl;
 ...
 */
  function filterFilter() {
    return function (array, expression, comparator) {
      ...
      let comparatorType = typeof (comparator),
        predicates = [];
        ...
        // jshint +W086
          for (const key in expression) {
            (function (path) {
              ...
              predicates.push(value => search(path == '$' ? value : (value && value[path]), expression[path]));
            }(key));
          }
          break;
        case 'function':
 ...
 */
  ...
  function numberFilter($locale) {
    ...
    return function (number, fractionSize) {
      return formatNumber(
        number, formats.PATTERNS[0], formats.GROUP_SEP, formats.DECIMAL_SEP,
        fractionSize
      );
    };
  }
  ...
  function formatNumber(number, pattern, groupSep, decimalSep, fractionSize) {
    ...
    } else if (fractionSize > 0 && number > -1 && number < 1) {
  ...
  function timeZoneGetter(date) {
    const zone = -1 * date.getTimezoneOffset();
  ...
  function dateFilter($locale) {
    ...
    function jsonStringToDate(string) {
      ...
      if (match = string.match(R_ISO8601_STR)) {
        ...
        const h = int(match[4] || 0) - tzHour;
        const m = int(match[5] || 0) - tzMin;
        const s = int(match[6] || 0);
 ...
 */
  ...
  function orderByFilter($parse) {
    return function (array, sortPredicate, reverseOrder) {
      ...
      sortPredicate = map(sortPredicate, (predicate) => {
        let descending = false, get = predicate || identity;
        if (isString(predicate)) {
          ...
          // Effectively no predicate was passed so we compare identity
            return reverseComparator((a, b) => compare(a, b), descending);
          }
          get = $parse(predicate);
          if (get.constant) {
            const key = get();
            return reverseComparator((a, b) => compare(a[key], b[key]), descending);
          }
        }
        return reverseComparator((a, b) => compare(get(a), get(b)), descending);
      });
      return slice.call(array).sort(reverseComparator(comparator, reverseOrder));

      function comparator(o1, o2) {
        for (let i = 0; i < sortPredicate.length; i++) {
          const comp = sortPredicate[i](o1, o2);
    ...
    compile(element, attr) {
      ...
      if (!attr.href && !attr.xlinkHref && !attr.name) {
        ...
        // SVGAElement does not use the href attribute, but rather the 'xlinkHref' attribute.
          const href = toString.call(element.prop('href')) === '[object SVGAnimatedString]' ?
            'xlink:href' : 'href';
 ...
 */
   ...
   */
    form.$removeControl = function (control) {
      ...
      forEach(errors, (queue, validationToken) => {
   ...
   */
    form.$setValidity = function (validationToken, isValid, control) {
      let queue = errors[validationToken];
  ...
  const formDirectiveFactory = function (isNgForm) {
    return ['$timeout', function ($timeout) {
      const formDirective = {
        ...
        compile() {
          return {
            pre(scope, formElement, attr, controller) {
              if (!attr.action) {
              ...
              // on a button in the form. Looks like an IE9 specific bug.
                const preventDefaultListener = function (event) {
                  event.preventDefault
                    ? event.preventDefault()
                    : event.returnValue = false; // IE
                };
              ...
              let parentFormCtrl = formElement.parent().controller('form'),
...
*/
  ...
  function textInputType(scope, element, attr, ctrl, $sniffer, $browser) {
    ...
    let placeholder = element[0].placeholder, noevent = {};
    ...
    if (!$sniffer.android) {
      ...
      element.on('compositionstart', (data) => {
        composing = true;
      });
    ...
    var listener = function (ev) {
      ...
      // a row.
      const revalidate = validity && ctrl.$$hasNativeValidators;
    ...
    if ($sniffer.hasEvent('input')) {
      ...
      element.on('keydown', (event) => {
        ...
        if (key === 91 || (key > 15 && key < 19) || (key >= 37 && key <= 40)) return;
    ...
    // pattern validator
    let pattern = attr.ngPattern,
      patternValidator,
      match;

    if (pattern) {
      const validateRegex = function (regexp, value) {
      ...
      if (match) {
        ...
        patternValidator = function (value) {
          const patternObj = scope.$eval(pattern);
    ...
    if (attr.ngMinlength) {
      const minlength = int(attr.ngMinlength);
      const minLengthValidator = function (value) {
    ...
    if (attr.ngMaxlength) {
      const maxlength = int(attr.ngMaxlength);
      const maxLengthValidator = function (value) {
  ...
  function numberInputType(scope, element, attr, ctrl, $sniffer, $browser) {
    ...
    ctrl.$formatters.push(value => (ctrl.$isEmpty(value) ? '' : `${value}`));
    ...
    ctrl.$formatters.push(value => validate(ctrl, 'number', ctrl.$isEmpty(value) || isNumber(value), value));
  }
  ...
  function checkboxInputType(scope, element, attr, ctrl) {
    let trueValue = attr.ngTrueValue,
    ...
    ctrl.$formatters.push(value => value === trueValue);
 ...
 *
 */
  const NgModelController = ['$scope', '$exceptionHandler', '$attrs', '$element', '$parse', '$animate',
    function ($scope, $exceptionHandler, $attr, $element, $parse, $animate) {
      ...
      let ngModelGet = $parse($attr.ngModel),
        ngModelSet = ngModelGet.assign;

      if (!ngModelSet) {
        throw minErr('ngModel')(
          'nonassign', "Expression '{0}' is non-assignable. Element: {1}",
   ...
   */
      ...
      let parentForm = $element.inheritedData('$formController') || nullFormCtrl,
        invalidCount = 0, // used to easily determine if we are valid
   ...
   */
      this.$setViewValue = function (value) {
        ...
        if (this.$modelValue !== value) {
          ...
          forEach(this.$viewChangeListeners, (listener) => {
            try {
              listener();
            } catch (e) {
              $exceptionHandler(e);
      ...
      $scope.$watch(() => {
        let value = ngModelGet($scope);

        // if scope model value and ngModel value are out of sync
        if (ctrl.$modelValue !== value) {
          let formatters = ctrl.$formatters,
 ...
 */
  var ngBindTemplateDirective = ['$interpolate', function ($interpolate) {
    ...
    // TODO: move this to scenario runner
      const interpolateFn = $interpolate(element.attr(attr.$attr.ngBindTemplate));
    ...
    return ['$animate', function ($animate) {
      return {
        ...
        link(scope, element, attr) {
          ...
          attr.$observe('class', (value) => {
            ngClassWatchAction(scope.$eval(attr[name]));
          });


          if (name !== 'ngClass') {
            scope.$watch('$index', ($index, old$index) => {
            // jshint bitwise: false
              const mod = $index & 1;
              if (mod !== (old$index & 1)) {
                const classes = arrayClasses(scope.$eval(attr[name]));
                mod === selector ?
                  addClasses(classes) :
          ...
          function addClasses(classes) {
            const newClasses = digestClassCounts(classes, 1);
          ...
          function digestClassCounts(classes, count) {
            const classCounts = element.data('$classCounts') || {};
          ...
          function updateClasses(oldClasses, newClasses) {
            let toAdd = arrayDifference(newClasses, oldClasses);
            let toRemove = arrayDifference(oldClasses, newClasses);
          ...
          function ngClassWatchAction(newVal) {
            if (selector === true || scope.$index % 2 === selector) {
              const newClasses = arrayClasses(newVal || []);
              if (!oldVal) {
                addClasses(newClasses);
              } else if (!equals(newVal, oldVal)) {
                const oldClasses = arrayClasses(oldVal);
      ...
      ngEventDirectives[directiveName] = ['$parse', '$rootScope', function ($parse, $rootScope) {
        return {
          ...
          // frequently as regular change detection.
            const fn = $parse(attr[directiveName], /* expensiveChecks */ true);
      ...
      return {
        ...
        compile(element, attr) {
          ...
          return function (scope, $element, $attr, ctrl, $transclude) {
            ...
            scope.$watch($sce.parseAsResourceUrl(srcExp), (src) => {
              ...
              if (src) {
                $http.get(src, { cache: $templateCache }).success((response) => {
  ...
  var ngPluralizeDirective = ['$locale', '$interpolate', function ($locale, $interpolate) {
    ...
    return {
      ...
      link(scope, element, attr) {
        ...
        scope.$watch(() => {
          let value = parseFloat(scope.$eval(numberExp));
      ...
      link($scope, $element, $attr, ctrl, $transclude) {
        ...
        if (!match) {
          throw ngRepeatMinErr(
            'iexp', "Expected expression in form of '_item_ in _collection_[ track by _id_]' but got '{0}'.",
            expression
          );
        }
        ...
        if (!match) {
          throw ngRepeatMinErr(
            'iidexp', "'_item_' in '_item_ in _collection_' should be an identifier or '(_key_, _value_)' expression, but got '{0}'.",
            lhs
          );
        }
        ...
        $scope.$watchCollection(rhs, (collection) => {
          ...
          for (key in lastBlockMap) {
            ...
            if (lastBlockMap.hasOwnProperty(key)) {
              ...
              forEach(elementsToRemove, (element) => { element[NG_REMOVED] = true; });
              block.scope.$destroy();
            }
          }
 ...
 */
  var ngStyleDirective = ngDirective((scope, element, attr) => {
    scope.$watch(attr.ngStyle, (newStyles, oldStyles) => {
      if (oldStyles && (newStyles !== oldStyles)) {
        forEach(oldStyles, (val, style) => { element.css(style, ''); });
      }
 ...
 */
        ...
        scope.$watch(watchExpr, (value) => {
          ...
          if ((selectedTranscludes = ngSwitchController.cases[`!${value}`] || ngSwitchController.cases['?'])) {
            ...
            forEach(selectedTranscludes, (selectedTransclude) => {
              const selectedScope = scope.$new();
              selectedScopes.push(selectedScope);
              selectedTransclude.transclude(selectedScope, (caseElement) => {
  ...
  var ngSwitchWhenDirective = ngDirective({
    ...
    link(scope, element, attrs, ctrl, $transclude) {
      ...
      ctrl.cases[`!${attrs.ngSwitchWhen}`].push({ transclude: $transclude, element });
    }
  });
 ...
 */
  ...
  var selectDirective = ['$compile', '$parse', function ($compile, $parse) {
    ...
    let NG_OPTIONS_REGEXP = /^\s*([\s\S]+?)(?:\s+as\s+([\s\S]+?))?(?:\s+group\s+by\s+([\s\S]+?))?\s+for\s+(?:([\$\w][\$\w]*)|(?:\(\s*([\$\w][\$\w]*)\s*,\s*([\$\w][\$\w]*)\s*\)))\s+in\s+([\s\S]+?)(?:\s+track\s+by\s+([\s\S]+?))?$/,
    ...
    return {
      ...
      // if ngModel is not defined, we don't need to do anything
        ...
        // find "null" option
        for (let i = 0, children = element.children(), ii = children.length; i < ii; i++) {
          if (children[i].value === '') {
        ...
        function setupAsSingle(scope, selectElement, ngModelCtrl, selectCtrl) {
          ngModelCtrl.$render = function () {
            const viewValue = ngModelCtrl.$viewValue;
        ...
        function setupAsMultiple(scope, selectElement, ctrl) {
          let lastView;
          ctrl.$render = function () {
            const items = new HashMap(ctrl.$viewValue);
            forEach(selectElement.find('option'), (option) => {
        ...
        function setupAsOptions(scope, selectElement, ctrl) {
          ...
          let displayFn = $parse(match[2] || match[1]),
          ...
          scope.$watchCollection(() => {
            ...
            if (values) {
              const toDisplay = new Array(values.length);
          ...
          if (multiple) {
            scope.$watchCollection(() => ctrl.$modelValue, render);
          }
          ...
          function getSelectedSet() {
            ...
            if (multiple) {
              ...
              if (trackFn && isArray(modelValue)) {
                ...
                const locals = {};
                for (let trackIndex = 0; trackIndex < modelValue.length; trackIndex++) {
          ...
          function render() {
            ...
            for (index = 0; length = keys.length, index < length; index++) {
              ...
              // either the index into array or key from object
                ...
                label,
                selected // determine if we should be selected
              });
            }
            if (!multiple) {
    ...
    return {
      ...
      compile(element, attr) {
        ...
        return function (scope, element, attr) {
          ...
          if (interpolateFn) {
            scope.$watch(interpolateFn, (newVal, oldVal) => {
